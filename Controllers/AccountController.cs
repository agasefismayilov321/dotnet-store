...
public class AccountController : Controller
{
    ...
    public async Task<ActionResult> Login(AccountLoginModel model, string? returnUrl)
    {
        if (ModelState.IsValid)
        {
            ...
            if(user != null)
            {
                ...
                {
                    ModelState.AddModelError("","Şifrə Hatalı");
                }
            }
            else
            {
    ...
    public async Task<ActionResult> ForgotPassword(string email)
    {
        ...
        if(user == null)
        {
            TempData["Message"] = "Bu eposta adresi kayıtlı deyil";
            return View();
        }

        var token = await _userManager.GeneratePasswordResetTokenAsync(user);
        var url = Url.Action("ResetPassword","Account",new {userId = user.Id, token});
        var link = $"<a href='http://localhost:5073{url}'>Şifre Yenile</a>";
        await _emailService.SendEmailAsync(user.Email!, "Parola Sıfırlama", link);
        
        TempData["Message"] = "Eposta adresine gönderilen link ile şifreni sıfırlaya bilersiniz";
